...
<!-- First filter and thenn clustering check -->
<!-- ```{r} -->
...
xenium.obj.female <- FindClusters(xenium.obj.female, resolution = 0.75)
```
...
Cardiomyocytes
```{r}
FeaturePlot(xenium.obj.female, c("Mylk3", "Trdn", "Hrc", "Myoz2"), label = T)
FeaturePlot(xenium.obj.female, c("Tnnc1", "Tnnt2-FSJ1"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
...
Fibroblasts
```{r}
FeaturePlot(xenium.obj.female, c("Dpt", "Lum"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```

Scahwann cells
```{r}
FeaturePlot(xenium.obj.female, "Plp1", label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```

EC cells - Coronary artery EC cells - using reverse engineering with Tabula Muris expression database
```{r}
FeaturePlot(xenium.obj.female, c("Aqp7", "Timp4"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```

Lymphatic ECs
```{r}
FeaturePlot(xenium.obj.female, c("Lyve1"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```

Erythrocytes & pericytes - reverse engineering
```{r}
FeaturePlot(xenium.obj.female, c("Higd1b", "Rsad2"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```

Endocardial
```{r}
FeaturePlot(xenium.obj.female, c("Vwf", "Plvap"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```

Epicardium
```{r}
FeaturePlot(xenium.obj.female, c("Upk3b"), label = T) & scale_color_distiller(name = stringr::str_wrap("Expression Level"), guide = guide_colourbar(title.position = "right")) &
  theme(legend.position = c(0.9,0.2),legend.title = element_text(angle = 90, size = 8 ,vjust = .1, hjust = .5), plot.background = element_rect(fill = "white")) & scale_colour_gradientn(colours = (brewer.pal(n = 6, name = "YlGnBu")))
```
...
Rename clusters
```{r}
new.cluster.ids <- c("ECs", "ECs", "CMs", "CMs", "Macrophage", "CMs", "Fibroblasts", "ECs", 
                     "Pericytes", "Fibroblasts", "Macrophage2", "CMs", "Fibroblasts", "Endocardial", "SMCs",
                     "Macrophage", "LymphaticEC", "MuscleContraction", "Neural", "Erythrocytes", "CMs", "ND", "Epicardial")
...
```{r}
xenium.obj.female <- subset(xenium.obj.female, idents = "ND", invert = T)
```
...
write.csv(clusters, "../r_objects/celltype_xenium_female_new.csv", quote = F, row.names = F)
```

<!-- Remove ND from cluster list -->
<!-- ```{r} -->
<!-- unique_clusters <- unique(Idents(xenium.obj.female)) -->
<!-- cluster_names <- unique_clusters[! unique_clusters %in% "ND"] -->
<!-- Idents(xenium.obj.female) <- factor(Idents(xenium.obj.female), levels = cluster_names) -->
<!-- ``` -->

## WT versus MI
Read `cell_IDs_samples` object from previous RDS saved data because it takes long time to run this.
```{r}
cell_IDs_samples <- readRDS("../r_objects/cell_IDs_samples.rds")
```
...
xenium.obj.female$condition <- factor(xenium.obj.female$condition, levels = c("WT", "MI", "ORAB1", "ORAB2")) # rearranging the order
```
...
markers_orab1 <- FindAllMarkers(xenium.obj.female_orab1)
write.csv(markers_orab1, file = "../r_objects/markers_ORAB1_female.csv", quote = F)
markers_orab2 <- FindAllMarkers(xenium.obj.female_orab2)
write.csv(markers_orab2, file = "../r_objects/markers_ORAB2_female.csv", quote = F)
```
...
```{r}
index <- which(Idents(xenium.obj.female_mi) == "MuscleContraction")
cells_mc <- names(Idents(xenium.obj.female_mi)[index])
ImageDimPlot(xenium.obj.female_mi, cells = cells_mc)
```

Image Dim Plots for each condition
```{r}
ImageDimPlot(xenium.obj.female_wt, dark.background = F, size = 1.5)
ImageDimPlot(xenium.obj.female_mi, dark.background = F, size = 1.5)
ImageDimPlot(xenium.obj.female_orab1, dark.background = F, size = 1.5)
ImageDimPlot(xenium.obj.female_orab2, dark.background = F, size = 1.5)
```
...
```{r}
a <- data.frame(group = "a", value = cells_female_seg$cell_area)
b <- data.frame(group = "b", value = cells_female$cell_area)
plot.data <- rbind(a, b)
...
xenium.obj.female_wt_niche <- BuildNicheAssay(object = xenium.obj.female_wt_niche, group.by = "celltype", niches.k = 3, neighbors.k = 25, fov = "fov")

new_niche_names <- list("1" = "test1", "2" = "test2", "3" = "test3") # rename zones with meaningfull names
xenium.obj.female_wt_niche$zones <- unlist(lapply(xenium.obj.female_wt_niche$niches, function(x) ifelse(x %in% names(new_niche_names), new_niche_names[x], "NA")))
ImageDimPlot(xenium.obj.female_wt_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches")
```
...
All graphics together for the wild-type
```{r}
ImageDimPlot(xenium.obj.female_wt_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches") | ggplot(niche_female_wt_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black")  + theme(legend.position="none")  + xlab("niches") | ggplot(niche_female_wt_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") + xlab("niches") 
```
...
xenium.obj.female_mi_niche <- BuildNicheAssay(object = xenium.obj.female_mi_niche, group.by = "celltype", niches.k = 3, neighbors.k = 25, fov = "fov")

new_niche_names <- list("1" = "border", "2" = "remote", "3" = "infarction") # rename zones with meaningfull names
xenium.obj.female_mi_niche$zones <- unlist(lapply(xenium.obj.female_mi_niche$niches, function(x) ifelse(x %in% names(new_niche_names), new_niche_names[x], "NA")))
ImageDimPlot(xenium.obj.female_mi_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches")
...
All graphics together for the MI heart
```{r}
ImageDimPlot(xenium.obj.female_mi_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches") | ggplot(niche_female_mi_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black")  + theme(legend.position="none")  + xlab("niches") | ggplot(niche_female_mi_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") + xlab("niches") 
```
...
xenium.obj.female_orab1_niche <- BuildNicheAssay(object = xenium.obj.female_orab1_niche, group.by = "celltype", niches.k = 3, neighbors.k = 25, fov = "fov")

new_niche_names <- list("1" = "border", "2" = "infarction", "3" = "remote") # rename zones with meaningfull names
xenium.obj.female_orab1_niche$zones <- unlist(lapply(xenium.obj.female_orab1_niche$niches, function(x) ifelse(x %in% names(new_niche_names), new_niche_names[x], "NA")))
ImageDimPlot(xenium.obj.female_orab1_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches")
...
All graphics together for the ORAB1 heart
```{r}
ImageDimPlot(xenium.obj.female_orab1_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches") | ggplot(niche_female_orab1_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black")  + theme(legend.position="none")  + xlab("niches") | ggplot(niche_female_orab1_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") + xlab("niches") 
```
...
xenium.obj.female_orab2_niche <- BuildNicheAssay(object = xenium.obj.female_orab2_niche, group.by = "celltype", niches.k = 3, neighbors.k = 25, fov = "fov")

new_niche_names <- list("1" = "remote", "2" = "border", "3" = "infarction") # rename zones with meaningfull names
xenium.obj.female_orab2_niche$zones <- unlist(lapply(xenium.obj.female_orab2_niche$niches, function(x) ifelse(x %in% names(new_niche_names), new_niche_names[x], "NA")))
ImageDimPlot(xenium.obj.female_orab2_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches")
```
...
All graphics together for the ORAB2 heart
```{r}
ImageDimPlot(xenium.obj.female_orab2_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches") | ggplot(niche_female_orab2_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black")  + theme(legend.position="none")  + xlab("niches") | ggplot(niche_female_orab2_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") + xlab("niches") 
```
...
```{r}
# WT
ImageDimPlot(xenium.obj.female_wt_niche, group.by = "niches", size = 1.5, dark.background = F, flip_xy = F) | ggplot(niche_female_wt_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches")
# MI
ImageDimPlot(xenium.obj.female_mi_niche, group.by = "niches", size = 1.5, dark.background = F, flip_xy = F) | ggplot(niche_female_mi_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches")
#ORAB1
ImageDimPlot(xenium.obj.female_orab1_niche, group.by = "niches", size = 1.5, dark.background = F, flip_xy = F) | ggplot(niche_female_orab1_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches")
# ORAB2
ImageDimPlot(xenium.obj.female_orab2_niche, group.by = "niches", size = 1.5, dark.background = F, flip_xy = F) | ggplot(niche_female_orab2_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches")
```

Abs population changes in ORAB1 and ORBA2 niches
```{r}
ggplot(niche_female_orab1_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches") | ggplot(niche_female_orab2_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches")
```

Abs population changes in ORAB1 and MI niches
```{r}
ggplot(niche_female_mi_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches") | ggplot(niche_female_orab1_abs_melt, aes(x=Var2, y=value, fill=Var1)) + geom_bar(stat = "identity", colour = "black") +  theme(text=element_text(size=16)) + ylab("Proportion of cell-types") + xlab("Niches")
```

MI and ORAB niches ImageDimPlot
```{r}
ImageDimPlot(xenium.obj.female_mi_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F) | ImageDimPlot(xenium.obj.female_orab1_niche, group.by = "zones", size = 1.5, dark.background = F, flip_xy = F)
```


Plotting niches as barplots (SCC-HD presentation)

Plot the cellular percentages for these two infarction niches as barplots i.e. niche 2 in MI and niche 2 in ORAB1
```{r}
niche_bar_mi_orab <- data.frame(niche_female_mi_abs[,"2"], niche_female_orab1_abs[,"2"])
...
Similarly check for infarction neighbor border zone i.e. niche 3 in MI and niche 1 in ORAB1
```{r}
niche_bar_mi_orab <- data.frame(niche_female_mi_abs[,"3"], niche_female_orab1_abs[,"1"])
colnames(niche_bar_mi_orab) <- c("MI", "ORAB1")
niche_bar_mi_orab$celltype <- rownames(niche_bar_mi_orab)
niche_bar_mi_orab_melt <- melt(niche_bar_mi_orab)
...
### All conditions combined (for DE analysis per niche in between different conditions)
```{r}
xenium.obj.female_niche <- xenium.obj.female
xenium.obj.female_niche$celltype <- as.vector(Idents(xenium.obj.female_niche))
xenium.obj.female_niche <- BuildNicheAssay(object = xenium.obj.female_niche, group.by = "celltype", niches.k = 3, neighbors.k = 25, fov = "fov")
# ImageDimPlot(xenium.obj.female_niche, group.by = "niches", size = 1.5, dark.background = F, flip_xy = F) + ggtitle("Niches")
```

Combine niches for each cell identified from different conditions SEPARATELY. Because combined information does not look accurate.
```{r}
list_niche_info_per_celltype <- list("wt" = data.frame(cell = rownames(xenium.obj.female_wt_niche@meta.data), zones = xenium.obj.female_wt_niche$zones), "mi" = data.frame(cell = rownames(xenium.obj.female_mi_niche@meta.data), zones = xenium.obj.female_mi_niche$zones), "orab1" = data.frame(cell = rownames(xenium.obj.female_orab1_niche@meta.data), zones = xenium.obj.female_orab1_niche$zones), "orab2" = data.frame(cell = rownames(xenium.obj.female_orab2_niche@meta.data), zones = xenium.obj.female_orab2_niche$zones))

niche_info_per_celltype <- do.call(rbind.data.frame, list_niche_info_per_celltype)
```

Find all genes that are different in each niche per celltype
```{r}
DefaultAssay(xenium.obj.female_niche) <- "SCT"
xenium.obj.female_niche$zones <- niche_info_per_celltype$zones[match(rownames(xenium.obj.female_niche@meta.data), niche_info_per_celltype$cell)] 
Idents(xenium.obj.female_niche) <-  xenium.obj.female_niche$zones
xenium.obj.female_niche$niche.condition <- paste(Idents(xenium.obj.female_niche), xenium.obj.female_niche$condition, sep="_")
Idents(xenium.obj.female_niche) <- "niche.condition"
condition.diffgenes <- list()
for (i in c("remote", "border", "infarction")){
  try({
    print(i)
    ident1 <- paste0(i,"_MI")
    ident2 <- paste0(i,"_ORAB1")
    condition.diffgenes[[i]] <- FindMarkers(xenium.obj.female_niche, ident.1 = ident1, ident.2=ident2)
  })
}

condition.diffgenes <- lapply(condition.diffgenes, function(x) cbind("genes"=rownames(x), x))   # for xls writing add rownames
write_xlsx(condition.diffgenes, path = paste0(output, "/DE_MIvsORAB1_perzone.xlsx"))
```
...
```{r}
bulk_female <- as.data.frame(PseudobulkExpression(object = xenium.obj.female, layer = "data")$SCT)
bulk_female$sum <- rowSums(bulk_female)
# filter out low expressed genes in all clusters
bulk_female <- bulk_female[bulk_female$sum >= 5,]
bulk_female$sum <- NULL
```

Heatmap
```{r}
library(pheatmap)
pheatmap(log2(bulk_female+1), angle_col = 315, fontsize_row = 7)
```

# Differential expression analysis between niches
Perform differential expression analysis between niches to identify genes specific for each niche
```{r}
# MI
de_niches_mi <- FindAllMarkers(xenium.obj.female_mi_niche, assay = "SCT", group.by = "niches")
de_niches_mi_filt <- de_niches_mi[de_niches_mi$p_val_adj < 0.05 & abs(de_niches_mi$avg_log2FC) > 1 ,]
# ORAB1
de_niches_orab1 <- FindAllMarkers(xenium.obj.female_orab1_niche, assay = "SCT", group.by = "niches")
de_niches_orab1_filt <- de_niches_orab1[de_niches_orab1$p_val_adj < 0.05 & abs(de_niches_orab1$avg_log2FC) > 1 ,]
# ORAB2
de_niches_orab2 <- FindAllMarkers(xenium.obj.female_orab2_niche, assay = "SCT", group.by = "niches")
de_niches_orab2_filt <- de_niches_orab2[de_niches_orab2$p_val_adj < 0.05 & abs(de_niches_orab2$avg_log2FC) > 1 ,]
```

Write DE genes into excel sheet
```{r}
list_de_niches <- list(de_niches_mi_filt, de_niches_orab1_filt, de_niches_orab2_filt)
names(list_de_niches) <- c("MI", "ORAB1", "ORAB2")
list_de_niches <- lapply(list_de_niches, function(x) cbind("genes"=rownames(x), x))   # for xls writing add rownames
write_xlsx(list_de_niches, path = paste0(output, "/niche_DE_genes.xlsx"))
```


# Trajectory analysis for all conditions
To see how transition of cardiomyocytes to macrophages occur, perform the trajectory analysis

Function for analysis
```{r}
trajectory_analysis <- function(xenium_obj){
  library(monocle3)
  library(RColorBrewer)
  library(Seurat)
  
  xenium_obj$celltype <- Idents(xenium_obj)
  data <- as(as.matrix(xenium_obj@assays$SCT@counts), 'sparseMatrix')
  cell_metadata <- data.frame(xenium_obj@meta.data)
  ...
  cds@int_colData@listData$reducedDims$UMAP <- xenium_obj@reductions$umap@cell.embeddings
  cds@clusters$UMAP$clusters <- xenium_obj@active.ident
...
Running trajectory function
```{r}
cds_wt <- trajectory_analysis(xenium_obj = xenium.obj.female_wt)
cds_mi <- trajectory_analysis(xenium_obj = xenium.obj.female_mi)
cds_orab1 <- trajectory_analysis(xenium_obj = xenium.obj.female_orab1)
```

Plotting trajectory (WT)
```{r}
#color by seurat cluster
plot_cells(cds_wt, label_groups_by_cluster = FALSE, color_cells_by = "celltype")
#color by monocle cluster
plot_cells(cds_wt, color_cells_by = "celltype", label_groups_by_cluster=FALSE, 
           label_leaves=F, label_branch_points=F, group_label_size = 3, trajectory_graph_color = "black")
plot_cells(cds_wt, color_cells_by = "pseudotime", label_groups_by_cluster=T, 
           label_leaves=F, label_branch_points=T)
# Plotting genes in pseudotime
plot_genes_in_pseudotime(cds_wt["Ankrd1-FSJ2",], color_cells_by="celltype", 
                              min_expr=0.5, ncol = 2)
```

Genes changing as the trajectory progresses
```{r}
# cds_mi_wt <- graph_test(cds_wt, neighbor_graph="principal_graph", cores=2)
cds_mi_de <- graph_test(cds_mi, neighbor_graph="principal_graph", cores=2)
cds_orab1_de <- graph_test(cds_orab1, neighbor_graph="principal_graph", cores=2)
# cds_orab2_de <- graph_test(cds_orab2, neighbor_graph="principal_graph", cores=2)
list_de_trajectory <- list("MI"=cds_mi_de, "ORAB1"=cds_orab1_de)
```

Plotting some top changing genes in MI
```{r}
plot_cells(cds_mi, genes=c("Col1a1-FSJ3", "Ctss", "Pln", "Anxa2"),
           show_trajectory_graph=T,
           label_cell_groups=FALSE,
           label_leaves=FALSE)
```

Plotting some top changing genes in ORAB1
```{r}
plot_cells(cds_orab1, genes=c("Col1a1-FSJ3", "Ctss", "Pln", "Anxa2"),
           show_trajectory_graph=T,
           label_cell_groups=FALSE,
           label_leaves=FALSE)
```

Save DE genes in file
```{r}
list_de_trajectory <- lapply(list_de_trajectory, function(x) cbind("gene_short_name"=rownames(x), x))   # for xls writing add rownames
write_xlsx(list_de_trajectory, path = paste0(output, "/DE_trajectory_MI+ORAB1.xlsx"))
```
